...
def get_output_file_name(model_name: str, max_change_per_step: float) -> str:
    return f"./results/{MODEL_NAME.split('/')[1]}_{'INF' if MAX_CHANGES_PER_STEP == np.inf else MAX_CHANGES_PER_STEP}.tsv"
...
SEED = 42
OUTPUT_FILE = get_output_file_name(model_name=MODEL_NAME, max_change_per_step=MAX_CHANGES_PER_STEP)
RTPT_NAME = "XX"
...
def main():
    ...
    dataset = load_csv_dataset(filename="./data/hate_speech_samples.tsv", delimiter="\t")
        ...
        while num_steps < MAX_STEPS:
            ...
            with torch.no_grad():
                ...
                if dist > MAX_CHANGES_PER_STEP or dist == 0:
                    ...
                    steps_without_update += 1
                    print(f"🚩 Distance {dist} is greater than threshold {MAX_CHANGES_PER_STEP}. Steps without update: {steps_without_update}")
